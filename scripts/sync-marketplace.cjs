...
/**
 * This fork's identity, read from the marketplace manifest rather than spelled
 * out again. `src/shared/plugin-identity.ts` is the source for the TypeScript
 * side, but a .cjs build script cannot import it — and a hand-copied literal
 * here is the exact drift 3ba70fdb..9ce48986 spent three commits removing.
 * `.claude-plugin/marketplace.json` is ground truth: it is the file Claude Code
 * itself reads, and the build now asserts it against PLUGIN_NAME/MARKETPLACE_NAME.
 */
function readIdentity() {
  const manifest = JSON.parse(
    readFileSync(path.join(__dirname, '..', '.claude-plugin', 'marketplace.json'), 'utf-8')
  );
  const marketplace = manifest.name;
  const plugin = manifest.plugins[0].name;
  if (!marketplace || !plugin) {
    throw new Error('.claude-plugin/marketplace.json is missing a marketplace or plugin name');
  }
  return { marketplace, plugin };
}

const { marketplace: MARKETPLACE_NAME, plugin: PLUGIN_NAME } = readIdentity();
const INSTALLED_PATH = path.join(os.homedir(), '.claude', 'plugins', 'marketplaces', MARKETPLACE_NAME);
const CACHE_BASE_PATH = path.join(os.homedir(), '.claude', 'plugins', 'cache', MARKETPLACE_NAME, PLUGIN_NAME);
