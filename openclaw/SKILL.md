...
If claude-mem is installed as a Claude Code plugin (at `~/.claude/plugins/marketplaces/max2535/`), start the worker from that installation:

```bash
cd ~/.claude/plugins/marketplaces/max2535
...
| Worker started from Claude Code install but not responding | Check `cd ~/.claude/plugins/marketplaces/max2535 && npm run worker:status`. May need `npm run worker:restart`. |
