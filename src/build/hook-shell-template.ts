 ...
 *   4. $_C/plugins/marketplaces/max2535/plugin (marketplace install)
 */
...
function candidateBlock(options: ShellTemplateOptions): string {
  ...
  const allGlobs = [...extraCacheRoots, '$_C/plugins/cache/max2535/claude-mem-pro-max']
    .map((root) => `"${root}"/[0-9]*/`)
    .join(' ');
  ...
  lines.push(`printf '%s\\n' "$_C/plugins/marketplaces/max2535/plugin";`);
...
function buildMcpNodeLauncher(options: ShellTemplateOptions): string {
  ...
  const cacheRoots = [
    ...
    '$_C/plugins/cache/max2535/claude-mem-pro-max',
  ].map(shTokenToNode);
  const marketplace = shTokenToNode('$_C/plugins/marketplaces/max2535/plugin');
...
export function buildCodexWindowsCommand(
  ...
  const parts = [
    ...
    "const cache=p.join(C,'plugins','cache','max2535','claude-mem-pro-max');",
    // S/W mirror compareVersionsDescending in src/shared/worker-utils.ts and
