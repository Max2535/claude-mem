 ...
 *   4. $_C/plugins/marketplaces/<marketplace>/plugin (marketplace install)
 */
...
} from '../shared/plugin-identity.js';
...
const MARKETPLACE_SEGMENT = `marketplaces/${MARKETPLACE_NAME}`;
...
function candidateBlock(options: ShellTemplateOptions): string {
  ...
  const allGlobs = [...extraCacheRoots, `$_C/plugins/cache/${CACHE_SEGMENT}`]
    .map((root) => `"${root}"/[0-9]*/`)
    .join(' ');
  ...
  lines.push(`printf '%s\\n' "$_C/plugins/${MARKETPLACE_SEGMENT}/plugin";`);
...
function buildMcpNodeLauncher(options: ShellTemplateOptions): string {
  ...
  const cacheRoots = [
    ...
    `$_C/plugins/cache/${CACHE_SEGMENT}`,
  ].map(shTokenToNode);
  const marketplace = shTokenToNode(`$_C/plugins/${MARKETPLACE_SEGMENT}/plugin`);
...
export function buildCodexWindowsCommand(
  ...
  const parts = [
    ...
    `const cache=p.join(C,'plugins','cache','${MARKETPLACE_NAME}','${PLUGIN_NAME}');`,
    // S/W mirror compareVersionsDescending in src/shared/worker-utils.ts and
    ...
    `roots.push(p.join(C,'plugins','marketplaces','${MARKETPLACE_NAME}','plugin'));`,
    "let R=null;",
