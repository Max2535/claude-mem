...
/**
 * The worker log console: filters, level/component chips, and the tailed
 * output. It carries no chrome of its own so the same console can sit inside
 * the floating drawer and inside the System screen without either one
 * growing a second copy of the parsing and filtering below.
 *
 * `onClose` is what makes it a drawer; without it there is nothing to close
 * and the button is not drawn.
 */
export function LogConsole({ onClose }: { onClose?: () => void }) {
  ...
  // Mounting is the signal to load: the drawer mounts this when it opens and
  // unmounts it when it closes, so there is no separate open flag to track.
  useEffect(() => {
